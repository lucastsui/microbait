...
export function briefingFromSources(goal, sources, extras = {}) {
  ...
  for (const source of sources || []) {
    ...
    const summary = neutralizeText(blob);
    const headline = neutralizeHeadline(source.title || "") || neutralizeHeadline(summary);
    if (!summary) {
    ...
    items.push({
      ...
      summary,
      relevance: relevanceLine(goal, summary || headline),
      sources: source.url
        ? [{ title: source.outlet || source.source || "Source", url: source.url }]
        : [],
      _score: relevanceScore(goal, `${headline} ${summary}`),
    });
  }

  items.sort((a, b) => b._score - a._score);
  const top = items.slice(0, 6).map(({ _score, ...rest }) => rest);
  const omittedParts = [];
...
function relevanceLine(goal, text) {
  const score = relevanceScore(goal, text);
  if (score >= 0.4) return `Matches the request: ${titleForGoal(goal)}.`;
  if (score > 0) return "Partly related to the stated request.";
  return "Included as nearby public context.";
}
